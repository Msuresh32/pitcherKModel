    ...
    for keys,g in raw.groupby(grp_cols, dropna=False):
        ...
        for book, bg in g.groupby('bookmaker'):
            ...
            if v is not None and np.isfinite(v):
                vals.append({'bookmaker':book,'under_close_cents':float(v)*100})
        choice=consensus_from_rows(pd.DataFrame(vals), 'under_close_cents') if vals else {'value':np.nan,'source':'missing','books':'','book_count':0,'confidence':'missing'}
        rows.append({
            ...
            'event_id':event_id,
            'sharp_under_close_cents': choice['value'],
...
def attach_close(pos, close):
    ...
    out['sharp_books']=''
    out['sharp_book_count']=np.nan
    out['sharp_confidence']='missing'
    for i,r in out.iterrows():
        ...
        if m.empty:
            continue
        row=m.sort_values(['sharp_book_count'], ascending=[False]).iloc[0]
        under=row['sharp_under_close_cents']
        side_close=under if r['net_side']=='UNDER' else 100-under if r['net_side']=='OVER' else np.nan
        out.loc[i,'sharp_source']=row['sharp_source']
        out.loc[i,'sharp_under_close_cents']=under
        out.loc[i,'sharp_side_close_cents']=side_close
        out.loc[i,'sharp_books']=row['sharp_books']
        out.loc[i,'sharp_book_count']=row['sharp_book_count']
        out.loc[i,'sharp_confidence']=row['sharp_confidence']
...
def main():
    ...
    print('SHARP_SOURCE_COUNTS')
    print(out['sharp_source'].value_counts(dropna=False).to_string())

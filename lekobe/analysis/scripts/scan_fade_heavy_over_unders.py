...
def choose_rows(market: pd.DataFrame, min_consensus_books: int) -> pd.DataFrame:
    ...
    rows = []
    for vals, group in market.groupby(key, dropna=False):
        event_id, clean_name, line = vals
        choice = consensus_from_rows(
            group.rename(columns={"no_vig_under": "sharp_under_prob"}),
            "sharp_under_prob",
        )
        rows.append(
            {
                ...
                "line": line,
                "consensus_under": choice["value"],
                "consensus_over": 1.0 - choice["value"] if np.isfinite(choice["value"]) else np.nan,
                "book_coverage": int(group["bookmaker"].nunique()),
    ...
    out["sharp_over"] = out["consensus_over"]
    out["sharp_under"] = out["consensus_under"]
    out["trigger_over_odds"] = out["pin_over_odds"]
    out["valid_benchmark"] = out["sharp_book_count"].gt(0) | (out["book_coverage"] >= min_consensus_books)
...
def scan(args: argparse.Namespace) -> pd.DataFrame:
    ...
    qualified["edge_buffer"] = np.where(qualified["sharp_book_count"].ge(2), args.pinnacle_edge_buffer, args.consensus_edge_buffer)

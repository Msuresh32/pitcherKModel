...
def choose_sharp_close(close_rows: pd.DataFrame, pitcher: str, line: float) -> dict:
    ...
    if cand.empty:
        return {"sharp_source": "missing", "match_type": "missing", "sharp_close_no_cents": np.nan}
    ...
    for book in PRIORITY:
        if book == "betonline":
            hit = pool[pool["bookmaker"].str.contains("betonline", na=False)]
        else:
            hit = pool[pool["bookmaker"].eq(book)]
        if not hit.empty:
            row = hit.sort_values("market_key").iloc[0]
            return {
                "sharp_source": book,
                "match_type": match_type,
                "matched_line": float(row["line"]),
                "hook_diff": float(row["line"]) - float(line),
                "close_over_odds": int(row["over_odds"]),
                "close_under_odds": int(row["under_odds"]),
                "sharp_close_no_cents": float(row["sharp_fair_no_cents"]),
                "commence_time": row.get("commence_time", ""),
            }
    # Consensus fallback over available books at same/nearest line.
    return {
        "sharp_source": "consensus",
        "match_type": match_type,
        "matched_line": float(pool["line"].iloc[0]),
        "hook_diff": float(pool["line"].iloc[0]) - float(line),
        "close_over_odds": np.nan,
        "close_under_odds": np.nan,
        "sharp_close_no_cents": float(pool["sharp_fair_no_cents"].mean()),
        "commence_time": pool["commence_time"].iloc[0] if "commence_time" in pool else "",
    }
...
def line_summary(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in raw.groupby(["event_sort", "event_start_pt", "game", "clean_name", "pitcher", "line"], dropna=False):
        ...
        fair_no = float(dedup["sharp_fair_no_cents"].mean())

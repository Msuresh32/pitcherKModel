...
def choose_close(raw: pd.DataFrame, pitcher: str, line: float) -> dict[str, object]:
    ...
    # First use an exact-line two-sided close from the highest-priority book.
    exact = pair_df[np.isclose(pair_df["matched_line"], float(line))]
    for book in PRIORITY:
        if book == "betonline":
            hit = exact[exact["bookmaker"].str.contains("betonline", na=False)]
        else:
            hit = exact[exact["bookmaker"].eq(book)]
        if not hit.empty:
            rec = hit.iloc[0].to_dict()
            rec["close_source"] = book
            rec["match_type"] = "exact"
            return rec

    # If no exact two-sided close exists, use nearest line from the priority book chain.
    for book in PRIORITY:
        if book == "betonline":
            hit = pair_df[pair_df["bookmaker"].str.contains("betonline", na=False)].copy()
        else:
            hit = pair_df[pair_df["bookmaker"].eq(book)].copy()
        if not hit.empty:
            hit["_abs_hook"] = hit["hook_diff"].abs()
            rec = hit.sort_values(["_abs_hook", "matched_line"]).iloc[0].drop(labels=["_abs_hook"]).to_dict()
            rec["close_source"] = book
            rec["match_type"] = "nearest"
            return rec

    # Last resort: consensus nearest line, averaging bookmaker-level no-vig under cents.
    ...
    return {
        "bookmaker": "consensus",
        "matched_line": float(nearest_line),
        "hook_diff": float(nearest_line) - float(line),
        "over_close": np.nan,
        "under_close": np.nan,
        "sharp_under_cents": float(g["sharp_under_cents"].mean()),
        "market": "consensus",
        "last_update": "",
        "close_source": "consensus",
        "match_type": "exact" if np.isclose(nearest_line, float(line)) else "nearest",
    }

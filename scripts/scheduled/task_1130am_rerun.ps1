...
$LogFile     = "$ProjectRoot\logs\rerun_1130_$Today.log"
...
function Ntfy($title, $body, $priority="high", $tags="baseball") {
...
if (Test-Path $CsvPath) {
    ...
    if ($picks) {
        ...
        $body = "PICKS ($Today):`n" + ($lines -join "`n")
        Ntfy "11:30am Rerun - $($picks.Count) picks" $body "high" "baseball,chart_with_upwards_trend"
        Log "Sent $($picks.Count) picks"
    } else {
        Ntfy "11:30am Rerun" "No 15%+ edge picks after lineup rerun" "default" "baseball"
        Log "No picks at edge>=15"
    }
}

& $Git -C $ProjectRoot add -A 2>&1 | Out-Null
& $Git -C $ProjectRoot commit -m "11:30am lineup rerun $Today" 2>&1 | Out-Null

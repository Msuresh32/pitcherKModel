...
def _catcher_framing_features(
    ...
    as a framing proxy. Returns (game_date, game_pk, catcher_csr_avg_prior,
    catcher_csr_excess_prior) keyed on game context.

    Catcher framing is the #3 highest-leverage missing feature (+0.3-0.8 K/9).
    Data source: statcast_catcher_framing_daily.csv (generated by fetch_statcast.py).
    Gracefully skips when the data file isn't available yet.
    """
    if (
        game_context_logs is None
        or game_context_logs.empty
        or catcher_framing_daily is None
        or catcher_framing_daily.empty
        or "catcher_take_csr" not in catcher_framing_daily.columns
    ):
        return pd.DataFrame()
    ...
    cf["game_date"] = pd.to_datetime(cf["game_date"])
    cf["catcher_id"] = cf["catcher_id"].astype(str)
    cf = cf.dropna(subset=["catcher_take_csr"])
    cf = cf.sort_values(["catcher_id", "game_date"])

    # League-wide daily baseline
    league_mean = cf.groupby("game_date")["catcher_take_csr"].mean().rename("_lg_csr")
    cf = cf.join(league_mean, on="game_date")

    # Per-catcher expanding prior mean (shift(1) to avoid leakage)
    grouped = cf.groupby("catcher_id", group_keys=False)
    shifted = grouped["catcher_take_csr"].shift(1)
    cf["catcher_csr_avg_prior"] = (
        shifted.groupby(cf["catcher_id"])
    ...
    shifted_lg = grouped["_lg_csr"].shift(1)
    lg_prior = (
        shifted_lg.groupby(cf["catcher_id"])
        .expanding(min_periods=10)
        .mean()
        .reset_index(level=0, drop=True)
    )
    cf["catcher_csr_excess_prior"] = cf["catcher_csr_avg_prior"] - lg_prior
    cf = cf.dropna(subset=["catcher_csr_avg_prior"])

    # Join back to game_context to get per-(game_pk, pitcher_id) catcher framing
    result = ctx.merge(
        cf[["game_date", "catcher_id", "catcher_csr_avg_prior", "catcher_csr_excess_prior"]],
        on=["game_date", "catcher_id"],
        how="left",
    ).drop_duplicates(subset=["game_date", "game_pk", "pitcher_id"])

    return result[
        ["game_date", "game_pk", "pitcher_id", "catcher_csr_avg_prior", "catcher_csr_excess_prior"]
    ].dropna(subset=["catcher_csr_avg_prior"])
...
def build_training_features(
    ...
    if catcher_framing_daily is not None and not catcher_framing_daily.empty:
        ...
        if not cf_features.empty and "game_pk" in df.columns:
            ...
            df = df.merge(
                cf_features,
                on=["game_date", "game_pk", "pitcher_id"],
                how="left",
            )
